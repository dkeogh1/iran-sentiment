...
case "$MODE" in
  push)
    ...
    for f in data/raw/truthsocial/replies_*.jsonl; do kubectl -n $NS cp "$f" "$POD:/data/raw/truthsocial/$(basename "$f")"; done
  ...
  pull)
    ...
    for f in $(kubectl -n $NS exec $POD -- sh -c 'cd /data/processed && ls teacher_check_* local_llm_* 2>/dev/null'); do
